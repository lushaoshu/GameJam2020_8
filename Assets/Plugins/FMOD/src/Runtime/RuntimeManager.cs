            ...
            get
            {
                ...
                if (instance == null)
                {
                    ...
                    try
                    {
                        ...
                        Settings.Instance.VerifyPlatformLibsExist();
...
retry:
        ...
        public static void PlayOneShotAttached(string path, GameObject gameObject,float volume=1)
        {
            try
            {
                PlayOneShotAttached(PathToGUID(path), gameObject, volume);
            }
            catch (EventNotFoundException)
            {
                Debug.LogWarning("[FMOD] Event not found: " + path);
        ...
        public static void PlayOneShotAttached(Guid guid, GameObject gameObject, float volume = 1)
        {
            var instance = CreateInstance(guid);
